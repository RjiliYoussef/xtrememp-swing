/**
 * Xtreme Media Player a cross-platform media player. Copyright (C) 2005-2010
 * Besmir Beqiri
 *
...
public abstract class AbstractButtonShaper implements SubstanceButtonShaper {
    ...
    @Override
    public final Dimension getPreferredSize(AbstractButton button, Dimension uiPreferredSize) {
        if (button.getClientProperty(SubstanceButtonUI.BORDER_COMPUTED) == null) {
            ...
            Insets borderIntets = border.getBorderInsets(button);
            if (!isBorderComputing) {
                button.setBorder(null);
            }
            uiPreferredSize.setSize(uiw - borderIntets.left - borderIntets.right,
                    uih - borderIntets.top - borderIntets.bottom);
            if (!isBorderComputing) {
